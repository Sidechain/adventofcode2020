...
const checkPasswordValidity = (passwords: Array<passwordProps>) => {
    ...
    passwords.forEach(e => {
        ...
        let first = e.lowestNumber-1
        let second = e.highestNumber-1
        let passwordArray: Array<String> = e.password.split("")

        if (passwordArray[first] == e.letter) isValid = !isValid
        if (passwordArray[second] == e.letter) isValid = !isValid
        if (isValid == true) validPasswords++
    })

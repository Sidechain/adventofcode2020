let passwords = `17-19 p: pwpzpfbrcpppjppbmppp
...
const parsePasswords = (passwordList: String) => {
    const passwordArray = passwordList.split("\n")
    passwordArray.forEach(e => {
        let splitArray = e.split(" ")

        let passwordWithProps: passwordProps = {
            lowestNumber: parseInt(splitArray[0].match(/^\d+/)[0]),
            highestNumber: parseInt(splitArray[0].match(/\d+$/)[0]),
            letter: splitArray[1].match(/^\w/)[0],
            password: splitArray[2]
        }
        parsedPasswords.push(passwordWithProps)
    })
}

const checkPasswordValidity = (passwords: Array<passwordProps>) => {
    let validPasswords = 0
    passwords.forEach(e => {
        let regexp = new RegExp(e.letter, "gi")
        let letterAmount

        if (e.password.match(regexp) == null) {
            letterAmount = 0
            console.log("0...")
        }
        else {
        letterAmount = e.password.match(regexp).length
        if (letterAmount > e.highestNumber) console.log("too many")
        if (letterAmount < e.lowestNumber) console.log("too few")
        if (letterAmount >= e.lowestNumber && letterAmount <= e.highestNumber) {
...
checkPasswordValidity(parsedPasswords)